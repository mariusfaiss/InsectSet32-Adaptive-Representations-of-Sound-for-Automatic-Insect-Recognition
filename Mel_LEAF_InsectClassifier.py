...
frontend = "LEAF"                        # define frontend for training or evaluation ("MEL", "LEAF")
...
num_epochs = 1                         # number of epochs for training when ES is disabled
...
N_MELS = 64                             # number of filters for Mel or LEAF
...
validation_data_path = f"{os.getcwd()}/Datasets/Validation Chunks/"    # Non-augmented validation dataset
test_data_path = f"{os.getcwd()}/Datasets/Test Chunks/"                # Non-augmented test dataset
...
class AudioUtil:
    ...
    def pre_leaf(aud):
        sig, sr = aud
        sig = torch.squeeze(sig)        # adapt input shape for LEAF

        if sig.size(dim=0) < sr * audio_len:    # pad signal if too short
            pad_value = (sig.size(dim=0) - sr * audio_len) * (-1)
            sig = F.pad(sig, (0, pad_value))

        if sig.size(dim=0) > sr * audio_len:    # truncate signal if too long
            sig = sig.numpy()
            sig = sig[:sr * audio_len]
            sig = torch.tensor(sig)
...
class SoundDS:
    ...
    def __getitem__(self, idx):
        ...
        if frontend == "LEAF":                          # process file for LEAF
            rep = AudioUtil.pre_leaf(aud)
...
class AudioClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        if frontend == "LEAF":
            x = x[:, None, :]           # add dummy dimension in middle to fit LEAF input requirement
            x = self.leaf(x)
            x = x[:, None, :, :]        # add another dummy dimension for input to next stage
